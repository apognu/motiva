...
fn score_feature(&self, lhs: &SearchEntity, rhs: &Entity) -> f64 {
  ...
  let lhs_addresses = HashSet::<String>::from_iter(lhs.property("publicKey").to_vec());
  let rhs_addresses = HashSet::<String>::from_iter(rhs.property("publicKey").to_vec());
...
mod tests {
  ...
  fn crypto_wallet_match() {
    ...
    assert_eq!(super::CryptoWalletMatch.score_feature(&lhs, &rhs), 1.0);
  }
}
