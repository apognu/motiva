...
async fn init_catalog<P: IndexProvider, F: CatalogFetcher>(fetcher: &F, provider: &P, outdated_grace: Span) -> Option<Catalog> {
  ...
  match get_merged_catalog(fetcher, provider, outdated_grace, true).await {
...
impl<P: IndexProvider, F: CatalogFetcher> Motiva<P, F> {
  ...
  pub async fn refresh_catalog(&self) -> Result<(), MotivaError> {
    match get_merged_catalog(&self.fetcher, &self.index, self.config.outdated_grace, true).await {
      Ok(catalog) => {
